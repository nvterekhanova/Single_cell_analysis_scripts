...
use.names = TRUE, unique.features = TRUE)
	metadata <-read.csv(file=paste(atac_data_folder,"/",sample,"/outs/per_barcode_metrics.csv",sep=""), 
...
#    	resolution = opt$joint_cluster_res,
	...
	p1 = DimPlot(pbmc, label = TRUE, repel = TRUE, reduction = "wnn.umap") +
    	NoLegend() +
    	ggtitle('Joint WNN')
	p2 = DimPlot(pbmc, reduction = 'rna.umap', label = TRUE,
             repel = TRUE, label.size = 2.5) +NoLegend() + ggtitle('RNA')
	p3 = DimPlot(pbmc, reduction = 'atac.umap', label = TRUE,
